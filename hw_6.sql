...
DELIMITER //
CREATE PROCEDURE move_user(id_to_move INT)

BEGIN

DECLARE user_firstname VARCHAR(50);
DECLARE user_lastname VARCHAR(50);
DECLARE user_email VARCHAR(120);
SET user_firstname = (SELECT u.firstname FROM users AS u WHERE u.id = id_to_move);
SET user_lastname = (SELECT u.lastname FROM users AS u WHERE u.id = id_to_move);
SET user_email = (SELECT u.email FROM users AS u WHERE u.id = id_to_move);

START TRANSACTION;

INSERT INTO users_old (id, firstname, lastname, email) VALUES
(id_to_move, user_firstname, user_lastname, user_email);

DELETE FROM users AS u WHERE u.id = id_to_move;
...
SET result = (SELECT CASE WHEN CURTIME() < '12-00-00' AND CURTIME() >= '06-00-00' THEN 'Доброе утро'
     WHEN CURTIME() < '18-00-00' AND CURTIME() >= '12-00-00' THEN 'Добрый день'
     WHEN CURTIME() < '00-00-00' AND CURTIME() >= '18-00-00' THEN 'Добрый вечер'
     WHEN CURTIME() < '06-00-00' AND CURTIME() >= '00-00-00' THEN 'Доброй ночи'
     END);
RETURN result;
END//
DELIMITER ;